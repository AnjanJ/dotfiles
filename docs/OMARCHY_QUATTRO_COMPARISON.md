...
> **Item 4 done (2026-09-03):** `dotfiles debug [--print] [--no-copy]` prints versions, the repo revision and dirt, the theme and toggles, what is waiting, every managed link's state, the colour-stripped health check and the update transcript tail; without `--print` it writes `~/.local/state/dotfiles/debug.log` and copies it with `pbcopy`. The skill and `AGENTS.md` lead with it, and `update.sh`'s failure hint names it. The symlink map gained `DOTFILES_OPTIONAL_LINKS`: the skill is also linked into `~/.agents/skills`, `~/.codex/skills` and `~/.gemini/config/skills` when that agent's directory exists.
...
> **Item 5 done (2026-09-03):** `dotfiles font current|list|set <family>|reset` records the family in `~/.local/state/dotfiles/font`; `apply-theme.sh` exposes it as `{{ font_family }}` (default Fira Code), a new `ghostty-font.tpl` renders `font-family` into `~/.config/ghostty/font.generated`, included by the tracked config, which no longer sets one (a separate file so a theme's hand-written Ghostty override cannot hide it), a new `wezterm-font.lua.tpl` is read by `~/.wezterm.lua` with the Nerd Font symbols fallback kept, Zed's `buffer_font_family` became a managed key of the generated settings, and VS Code's base carries an `editor.fontFamily` placeholder. `set` verifies the family with `fc-list` when fontconfig is installed, re-renders the active theme and fires `font-set`. WezTerm and Zed's editor buffer move from JetBrains Mono to the shared default; `dotfiles font set` changes it everywhere at once.
...
> **Item 6 done (2026-09-03):** `hooks/<event>.d/*.sample` in the repo (notify on theme-set, post-update and post-install, a sync log for post-sync) are seeded into `~/.config/dotfiles/hooks/` by `install.sh`, `dotfiles doctor` and `dotfiles hook --seed`, never overwriting and respecting a deleted sample; `dotfiles hook --list` shows what is installed per event; `dotfiles hook install <event> <file>` copies a script in (port of `omarchy-hook-install`); `install.sh` fires `post-install` after the health check. The docs suite already checks that every fired event is in the help and the skill.
...
> **Item 7 done (2026-09-03):** `dotfiles theme update [name] [--no-apply]` fast-forwards every clone under `~/.config/dotfiles/themes/` (a diverged clone is reported, not clobbered) and re-applies the active theme when it moved; `dotfiles theme preview [name|colors.toml] [--no-color]` renders the palette through `theme-render.sh` and prints true-colour swatches, and `dotfiles menu theme` shows it in an fzf preview pane; `apply_theme` now takes the same `mkdir` lock with stale-pid detection that `update.sh` uses, wrapping the old body as `_apply_theme_locked`.
...
> **Item 8 done (2026-09-03):** `dotfiles keys --lint` (a CI step next to `--check`) fails on a chord bound twice within a mode regardless of modifier order, a binding whose command the cheatsheet cannot turn into a sentence and that has no `# desc:`, and an `exec-and-forget` script under `~/.config/aerospace/scripts/` or `~/bin/` that the repo does not ship. The real config lints clean; the keys suite covers each failure. Launcher bundles are a machine fact, not a repo fact, so they stay with `dotfiles health`.
...
> **Item 11, usage half, done (2026-09-03):** `dotfiles agent usage` reads the token Claude Code keeps in the keychain (or `~/.claude/.credentials.json`), calls the OAuth usage endpoint, caches the reply for a minute under `~/.cache/dotfiles`, and prints a summary, `--json`, a `--bar` line, or a `--notify` notification; the weekly figure is the fullest weekly limit, per-model ones included, and the token is never refreshed. `plugins/agent-usage.sh` paints `5h 86% · 7d 25%` (yellow at 70%, red at 90%, grey when stale, hidden without a login) behind `dotfiles toggle agent-usage`, and a click posts the reset times. Written against the endpoint directly rather than ported: the Omarchy script is no longer in that repo's tree. The crash watcher is not built.
